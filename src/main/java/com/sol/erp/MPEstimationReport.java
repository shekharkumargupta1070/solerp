...
public class MPEstimationReport implements java.awt.event.ActionListener {
	...
	JInternalFrame f = new JInternalFrame("View MP Estimation Report", true, true, true, true);
	...
	JLabel title = new JLabel(new javax.swing.ImageIcon("image/pm.gif"));
	...
	public void projectNo() {
		...
		try {
			...
			rs = stat.executeQuery("select Distinct(Project_No) from Estimation_MP");
	...
	public void details() {
		...
		try {
			...
			rs = stat.executeQuery(
					"select Project_name,Client_name,Item_code,Item_Qty,Sheet_Size,Sheet_Qty,Dtl_Time,Chk_Time,PerSheet_Time,Total_Time,Complexity from Estimation_MP where Project_no='"
							+ tf1.getSelectedItem() + "' ");
			while (rs.next()) {
				...
				model.addRow(new Object[] { str3, String.valueOf(df3.format(flstr4)), str4,
						String.valueOf(df3.format(flstr6)), String.valueOf(flstr7), String.valueOf(flstr8), str5,
						String.valueOf(flstr10), String.valueOf(flstr11) });
			...
			model.addRow(new Object[] { "", String.valueOf(df3.format(sumitemqty)), "",
					String.valueOf(df3.format(sumsheetqty)), ttldtl, ttlchk, "", ttlhrs, ttlcom });
	...
	public void summary() {
		try {
			...
			while (rs.next()) {
				...
				model.addRow(
						new Object[] { "", str1, "", str2, String.valueOf(df.format(f1)), String.valueOf(df.format(f2)),
